...
// 算法文件生成exe的路径
const string S_EXE = R"(..\cmake-build-debug\baseline_main.exe)";
// 算法文件的路径
const string S_CPP = R"(..\src\baseline\baseline_main.cpp)";
// check_main.exe的路径
const string C_EXE = R"(..\cmake-build-debug\check_main.exe)";
// check_main.cpp的路径
const string C_CPP = R"(..\src\baseline\check_main.cpp)";

// 图数据集个数
const int TEST_CASE_NUM = 5;

/**
 * 从管道中读取数据，直到没有更多的数据为止
 */
string readPipeData(HANDLE hPipe) {
    const size_t bufferSize = 655360;  // 缓冲区大小，65536字节
...
int main() {
    const size_t bufferSize = 655360; // 缓冲区大小
    ...
    for (int param = 1; param <= TEST_CASE_NUM; ++param) {
        ...
        string cmd = C_EXE + " " + to_string(param); // 添加参数
        if (!CreateProcess(NULL, const_cast<char*>(cmd.c_str()), NULL, NULL, TRUE, 0, NULL, NULL, &si2, &pi2)) {
